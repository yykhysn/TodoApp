...
def test_update_user(add_test_data, test_db):
    ...
    response = client.put("/api/user/update", json=UsersUpdateSchema(old_password=str(uuid.uuid4().hex),
    ...
    response = client.put("/api/user/update", json=UsersUpdateSchema(old_password=old_password,
                                                                 new_password=new_password).model_dump(exclude_unset=True))
    ...
    # Test update email:
    response = client.put("/api/user/update", json=UsersUpdateSchema(email=f"{user_name}@change_email.user").
    ...
    response = client.put("/api/user/update", json=UsersUpdateSchema(first_name="new_first_name").model_dump(exclude_unset=True))
    ...
    response = client.put("/api/user/update", json=UsersUpdateSchema(last_name="new_last_name").model_dump(exclude_unset=True))
    ...
    response = client.put("/api/user/update",json=UsersUpdateSchema(is_enabled=False).model_dump(exclude_unset=True))
...
def test_login_user(add_test_data):
    ...
    response = client.post("/api/user/login", data={"username": test_user_username, "password": test_user_password})
    assert response.status_code == 200
    response_json = response.json()
